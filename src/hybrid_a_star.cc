...
std::shared_ptr<Node3d> HybridAStar::Next_node_generator(
  ...
  std::vector<double> intermediate_x_1;
  std::vector<double> intermediate_y_1; 
  intermediate_x_1.push_back(intermediate_x.back());
  intermediate_y_1.push_back(intermediate_y.back()); 
...
bool HybridAStar::Plan(
  ...
  std::vector<double> Debug_Ob_x, Debug_Ob_y;
  ...
  for (const auto& obstacle_vertices : obstacles_vertices_vec) {
    ...
    for (size_t i = 0; i < vertices_num - 1; ++i) {
      ...
      Debug_Ob_x.push_back(obstacle_linesegments[i].start.x);
      Debug_Ob_x.push_back(obstacle_linesegments[i].end.x);
      Debug_Ob_y.push_back(obstacle_linesegments[i].start.y);
      Debug_Ob_y.push_back(obstacle_linesegments[i].end.y);
